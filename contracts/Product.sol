...
/* @title Product Contract
   @author Abdallah-Hossayn Mokhtar Bacha
...
contract Product {
  ...
  /* @notice Constructor to create a Product
    ...
    function addAction(
        //bytes32 type,
        bytes32 description,
        bytes32 geoUri) public notDelivered {
...
contract ProductFactory {
    ...
    /* @notice Function to create a Product
       ...
       @param _geoUri Location of the created Product.
       @param _dataBaseContract Reference to its database contract */
    function createProduct(
        ...
        address newProduct = new Product(_name, _additionalInformation, _geoUri, _dataBaseContract, this);
        ProductOwner memory newlyCreatedProduct;
        newlyCreatedProduct.product = newProduct;
        newlyCreatedProduct.owner = msg.sender;
        products.push(newlyCreatedProduct);
        NewProductCreated(newProduct, msg.sender, now);
        return newProduct;
    }
}
